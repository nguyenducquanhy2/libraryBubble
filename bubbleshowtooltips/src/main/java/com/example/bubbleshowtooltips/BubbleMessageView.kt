...
class BubbleMessageView : ConstraintLayout {
    ...
    private fun setAttributes(builder: Builder){
        ...
        targetView= builder.mTargetView?.get()
        mActivity=builder.mActivity.get()
...
//        drawArrow(canvas,  BubbleShowCase.ArrowPosition.BOTTOM, targetViewScreenLocation)
    ...
    private fun drawRectangle(canvas: Canvas, x: Int, y: Int) {
        ...
        var xOfNewConstraintLayout=x.toFloat()-showCaseMessageViewLayout!!.width.toFloat()/2

        if (xOfNewConstraintLayout<0){
            xOfNewConstraintLayout=MARGIN_LEFT_SCREEN
            Log.e("toaDoManHinh", "tọa độ mới x constrainlayout kịch trái: ${xOfNewConstraintLayout}" )
        }
        else if (xOfNewConstraintLayout+showCaseMessageViewLayout!!.width>screenWidth){
            xOfNewConstraintLayout-= xOfNewConstraintLayout+showCaseMessageViewLayout!!.width-screenWidth

            Log.e("toaDoManHinh", "tọa độ x constrainlayout kịch phải: ${xOfNewConstraintLayout}" )
        }

        showCaseMessageViewLayout!!.translationX=xOfNewConstraintLayout
...
//        xPosition=177
//        yPosition=229
        drawRectangle(canvas,xPosition, yPosition)
        drawRhombus(canvas,arrowPosition, paint, xPosition, yPosition, ScreenUtils.dpToPx(WIDTH_ARROW))
    }
    ...
    private fun drawRhombus(canvas: Canvas,arrowPosition: BubbleShowCase.ArrowPosition, paint: Paint?, x: Int, y: Int, width: Int) {
        ...
        Log.e("toaDoManHinh", "tọa độ x top view target: ${targetView!!.translationX}" )
        Log.e("toaDoManHinh", "tọa độ y top view target: ${targetView!!.translationY}" )


        val path = Path()

        if (arrowPosition == BubbleShowCase.ArrowPosition.BOTTOM || arrowPosition == BubbleShowCase.ArrowPosition.TOP) {
            val widthNarrownessRhombus = ScreenUtils.dpToPx(WIDTH_NARROWNESS_RHOMBUS)

            path.moveTo(x.toFloat(), (y + halfRhombusWidth).toFloat()) // Top
            path.lineTo(
                (x - halfRhombusWidth + widthNarrownessRhombus).toFloat(),
                y.toFloat()
            ) // Left
            ...
            val heightNarrownessRhombus = ScreenUtils.dpToPx(HEIGHT_NARROWNESS_RHOMBUS)
            path.moveTo(
                x.toFloat(),
                (y + halfRhombusWidth - heightNarrownessRhombus).toFloat()
            ) // Top
    ...
    class Builder{
        ...
        var mTargetView: WeakReference<View>? =null
        ...
        fun activity(activity: WeakReference<Activity>): Builder{
            mActivity = activity
            return this
        }
        ...
        fun targetView(targetView:WeakReference<View>?): Builder {
